...
def test_get_generic_service_class(ioc):
    ioc.register(Service, Scope.SINGLETON, provides=GenericService[int])

    service = ioc.get(GenericService[int])
    assert isinstance(service, Service)


def test_get_generic_service_factory(ioc):
...
def test_get_generic_service_class_with_dependency(ioc):
    ioc.register(Service, Scope.SINGLETON, provides=GenericService[int])
    ioc.register(ServiceDepends, Scope.SINGLETON)

    service = ioc.get(ServiceDepends)
    assert isinstance(service, ServiceDepends)
    assert isinstance(service.service, Service)


def test_get_generic_service_factory_with_dependency(ioc):
    ioc.register(service_factory, Scope.SINGLETON)
...
def test_service_provides_wrong_generic_type_should_fail(ioc):
    U = TypeVar("U")
...
def test_service_provides_wrong_generic_parameter_should_fail(ioc):
    with pytest.raises(IncompatibleTypesError):
        ioc.register(Service, Scope.SINGLETON, provides=GenericService[str])


def test_type_var_in_generic_service_should_fail(ioc):
